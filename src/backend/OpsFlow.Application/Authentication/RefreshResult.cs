...
/// <summary>
/// The result of the Application refresh use case. Use the <see cref="Success"/>
...
/// OpsFlow.Api maps it to a <c>RefreshResponse</c>: the refresh token is removed
...
public sealed record RefreshResult
{
    ...
    /// <summary>Whether the refresh attempt succeeded.</summary>
    public bool Succeeded { get; }

    /// <summary>The freshly signed access token on success; <c>null</c> on failure.</summary>
    ...
    /// <summary>
    /// The raw successor refresh token on success; <c>null</c> on failure.
    /// Transient server-side data: never persisted, logged, or serialized in
    /// JSON, and written only to an HttpOnly cookie by OpsFlow.Api.
    /// </summary>
    ...
    /// <param name="refreshToken">The raw successor refresh token. Must not be null, empty, or whitespace.</param>
    ...
    /// <summary>Creates a failed refresh result with no tokens, expirations, or user.</summary>
